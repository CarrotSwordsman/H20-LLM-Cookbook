...
fused MoE Triton kernel, on representative (E, N) shapes and batch sizes.

Output: markdown table suitable for posting to vLLM PR #44152.
...
# Reuse the existing benchmark harness in this directory.
HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(HERE))
from tune_moe_h20 import benchmark_one  # noqa: E402
...
# Representative shapes from the 12 newly tuned configs in PR #44152.
SHAPES = [
...
def load_tuned_config(E: int, N: int) -> dict:
    """Load the tuned config JSON we generated for H20."""
    # Look in the cookbook's own configs/bf16/ directory first; fall back
    # to vLLM's source tree layout for users who symlinked into vllm.
    cookbook_dir = HERE.parent.parent / "configs" / "bf16"
    candidates = [
        cookbook_dir / f"E={E},N={N},device_name=NVIDIA_H20.json",
        HERE.parent / "vllm" / "model_executor" / "layers" / "fused_moe" /
            "configs" / f"E={E},N={N},device_name=NVIDIA_H20.json",
    ]
    ...
    raise FileNotFoundError(
        f"No tuned H20 bf16 config for (E={E}, N={N}); tried {candidates}")
...
def main():
    print(f"# fused MoE kernel: default-fallback vs tuned H20 config")
    ...
    for E, N, K, topk, label in SHAPES:
        ...
        for bs in BATCH_SIZES:
            ...
            try:
                t_def = [benchmark_one(default, bs, E, N, K, topk, torch.bfloat16) for _ in range(3)]
                t_default = median_of(t_def)

                t_tun = [benchmark_one(tuned, bs, E, N, K, topk, torch.bfloat16) for _ in range(3)]
